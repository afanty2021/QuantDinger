...
> **更正（2026-09-13）**：本表"暴露价格语义"行依据的"现有腾讯层同为前复权（qfq）"假设经实测**证伪**——investment_data 存储的是归一化累计复权（factor ≈ 0.24 且最新行也不为 1），原样暴露会与在线层量级差 ~4.1×。已在 `qlib-qfq-exposure-20260913.md` 设计并实施修正：暴露价改为锚定包尾前复权（`stored ÷ f_last`，最新行 = 原始价），成交量还原原始量（`stored × f[t]`）。
